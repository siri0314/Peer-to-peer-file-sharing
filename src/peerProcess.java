...
class PeerInfo extends Thread{
    ...
    public void set_Peer_ID(int peer_ID) {
    ...
    public int get_Have_File() {
        return have_File;
    }

    public void set_Have_File(int have_File) {
    ...
    public void set_Bit_field(int[] bit_field) {
        this.bit_field = bit_field;
    }

    public void update_Bit_field(int index){
        bit_field[index] = 1;
    }
}
    ...
    */
        ...
        public void run(){
            while(completed_Peers < peers.size()){
                ...
                if(thisPeer.get_Have_File() == 1) {
                    ...
                    int counter = 0;
                    for (int peer : connections) {
                        if(peer_Connections.get(peer).is_Interested() && peer_Connections.get(peer).get_DownloadRate() >= 0)
                            interested_Peers.add(peer);
                    }
                    if(interested_Peers.size() <= config_Info.get_Number_Of_Preferred_Neighbors()){
    ...
    private static class Receive_Connections extends Thread{
        ...
        public void run(){
            ...
            try{
                ServerSocket serverSocket = new ServerSocket(thisPeer.get_Port_Number());
                while(peer_Connections.size() < peers.size() - 1){
                    ...
                    StringBuilder hand_shake_Msg = new StringBuilder();
                    hand_shake_Msg.append(new String(Arrays.copyOfRange(buffer, 0, 28)));
                    hand_shake_Msg.append(peer_ID);
    ...
    private static class Peer_Connection{
        ...
        public void compareBit_field(int[] thisPeerBit_field, int[] connectedPeerBit_field, int len){
            int i;
            for(i = 0; i < len; i++){
        ...
        public void get_Piece_Index(int[] thisPeerBit_field, int[] connectedPeerBit_field, int len){
        ...
        public void check_Completed(){
            ...
            if(counter == thisPeer.get_Bit_field().length){
                        ...
                        while(completed_Peers < peers.size()){
                            ...
                            switch (msgType){
                                ...
                                case HAVE:
                                    ...
                                    peer.compareBit_field(thisPeer.get_Bit_field(), peers.get(peer.get_Peer_ID()).get_Bit_field(), thisPeer.get_Bit_field().length);
                                ...
                                case BIT_FIELD:
                                    ...
                                    peer.compareBit_field(thisPeer.get_Bit_field(), bit_field, bit_field.length);
                                    break;
                                case REQUEST:
